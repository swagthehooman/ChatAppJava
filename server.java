...
public class server implements ActionListener {
	...
	// private client obj = new client("127.0.0.1");

	public server(String name) {

		serverframe = new JFrame(name);
		serverframe.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		serverframe.setSize(new Dimension(1000, 720));
			...
			try {
				if (c == 1) {
					...
					for (int i = 0; i < original.length(); i++) {
						...
						if (x == 32) {
							if (!word.equalsIgnoreCase("client:") && !word.equalsIgnoreCase("incomingfile:")
	...
	private void closeup() {
		showmessage("Closeing connections\n");
		abletotype(false);
		try {
			output.close();
			input.close();
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == fileselection) {
			...
			int response = filechoice.showOpenDialog(serverframe.getParent());
		...
		if (e.getSource() == usertext) {
			// obj.sendmessage(e.getActionCommand());
		}
